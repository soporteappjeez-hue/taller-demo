...
function drawHeader(doc: jsPDF, title: string, subtitle: string) {
  ...
  doc.text("AppJeez", 14, 14);

  // Subtitle in bar
  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.text("Demo para Service Tecnicos", 60, 14);
...
function drawFooter(doc: jsPDF) {
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    doc.setFontSize(7);
    doc.setTextColor(156, 163, 175);
    doc.text("AppJeez — Sistema de Gestión de Comercios", 14, ph - 3.5);
...
export function exportOrdersReportPDF(orders: WorkOrder[], filterLabel = "Todas las órdenes") {
  ...
  doc.save(`Reporte_Ordenes_AppJeez_${fecha}.pdf`);
}
...
export function exportOrderDetailPDF(o: WorkOrder) {
  ...
  y += 5;
  const msg = `Hola ${o.clientName}, te informamos que tu ${o.brand} ${o.model} (${o.motorType}) ya está lista para ser retirada. ¡Gracias por confiar en nosotros!`;
